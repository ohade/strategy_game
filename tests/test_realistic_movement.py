...
from game_logic import calculate_rotation, apply_rotation_inertia
from unit_mechanics import smooth_movement

class TestRealisticMovement(unittest.TestCase):
    ...
    def test_calculate_rotation_returns_correct_angle(self):
        ...
        self.assertTrue(math.isclose(angle, 45, abs_tol=0.1))
        
        # Test another angle
        target_x, target_y = 0, 10  # 90 degrees
        angle = calculate_rotation(start_x, start_y, target_x, target_y)
        
        self.assertTrue(math.isclose(angle, 90, abs_tol=0.1))
    ...
    def test_calculate_rotation_handles_all_quadrants(self):
        ...
        target_x, target_y = -10, 10
        angle = calculate_rotation(start_x, start_y, target_x, target_y)
        self.assertTrue(math.isclose(angle, 135, abs_tol=0.1))
        
        # Third quadrant
        target_x, target_y = -10, -10
        angle = calculate_rotation(start_x, start_y, target_x, target_y)
        self.assertTrue(math.isclose(angle, 225, abs_tol=0.1))
        
        # Fourth quadrant
        target_x, target_y = 10, -10
        angle = calculate_rotation(start_x, start_y, target_x, target_y)
        self.assertTrue(math.isclose(angle, 315, abs_tol=0.1))
    ...
    def test_apply_rotation_inertia_gradually_rotates_towards_target(self):
        ...
        self.assertTrue(math.isclose(new_rotation, 45, abs_tol=0.1))
        
        # Second update (should rotate by max speed again)
        unit.rotation = new_rotation
        new_rotation = apply_rotation_inertia(unit.rotation, target_angle, max_rotation_speed)
        self.assertTrue(math.isclose(new_rotation, 90, abs_tol=0.1))
    ...
    def test_apply_rotation_inertia_handles_angle_wrapping(self):
        ...
        self.assertTrue(math.isclose(new_rotation, target_angle, abs_tol=0.1))
    ...
    def test_smooth_movement_applies_inertia(self):
        ...
        self.assertGreater(unit.velocity_x, 0)  # Should have positive x velocity
        self.assertLessEqual(unit.velocity_x, unit.acceleration * dt)  # Should not exceed acceleration
        ...
        self.assertGreater(unit.velocity_y, 0)  # Should start moving upward
